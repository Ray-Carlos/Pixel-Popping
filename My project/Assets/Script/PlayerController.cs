...
public class PlayerController : MonoBehaviour
{
    ...
    void movePress()
    {
        if (Keyboard.current.aKey.wasPressedThisFrame)//判断这个按键是否按下。 wasPressedThisFrame 按下
        {
            moveDir = -1.0f;
        }
        if (Keyboard.current.sKey.wasPressedThisFrame)//判断这个按键是否按下。 wasPressedThisFrame 按下
        {
            moveDir = 1.0f;
        }
        if (!Keyboard.current.aKey.isPressed && !Keyboard.current.sKey.isPressed)//判断这个按键是否按下。 wasPressedThisFrame 按下
        {
    ...
    void Jump()
    {
        if (Keyboard.current.fKey.wasPressedThisFrame && jumpRestTimes >= 1) 
        {
            jumpRestTimes--;
